...
export class LittleCoreService {
  ...
  async getStats(ids: number[]) {
    ...
    const playersStats = (await response.json()) as any;
  ...
  playerStatsMapper(playerStatsBE: StatsBE): Stats {
    return {
      ...
      gp: playerStatsBE.game.period,
      player: `${playerStatsBE.player.first_name} ${playerStatsBE.player.last_name}`,
      team: playerStatsBE.team.name,
    };
  }
}
